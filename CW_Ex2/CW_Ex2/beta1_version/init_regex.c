...
const char *FLAGS_STR_ARRAY[] = {"-i", "-v", "-c", "-n",
                                 "-x", "-b", "-E", "-A"};

Error_Code_t
init_regex_block_and_advance_expression(Regex_Block *current_regex_block,
                                        char **p_rest_of_expression);
int get_bracket_block_end(char *expression, int index);
Error_Code_t
set_parentheses_block_return_status(char **p_parentheses_block_start,
                                    Regex_Block *current_regex_block);
void set_parentheses_block_contents(Regex_Block_Contents *regex_block_contents,
                                    char *left_side, char *right_side);
char *set_bracket_block(char *bracket_block_start,
                        Regex_Block *current_regex_block);
void set_regular_char_block(char regular_char,
                            Regex_Block *current_regex_block);
...
Error_Code_t initialize_regex_array(char *expression,
                                    Regex_Block **p_regex_array,
                                    int *p_regex_array_len, bool is_E_flag) {
  Error_Code_t status = SUCCESS_CODE;
  int index;
  *p_regex_array_len = count_amount_of_regex_blocks(expression);
  *p_regex_array =
      (Regex_Block *)malloc(sizeof(Regex_Block) * (*p_regex_array_len));
  ...
  for (index = 0; index < (*p_regex_array_len); index++) {
    status = init_regex_block_and_advance_expression(&(regex_array[index]),
                                                     &rest_of_expression);
...
init_regex_block_and_advance_expression(Regex_Block *current_regex_block,
  ...
  case CHAR_REGULAR:
    set_regular_char_block(rest_of_expression[FIRST_CHAR_IN_STR],
                           current_regex_block);
    break;

  case CHAR_BACK_SLASH:
    set_regular_char_block(rest_of_expression[SECOND_CHAR_IN_STR],
                           current_regex_block);
    rest_of_expression++;
    break;
  ...
  case CHAR_PARENTHESES_START:
    rest_of_expression++;
    status = set_parentheses_block_return_status(&rest_of_expression,
                                                 current_regex_block);
    break;

  case CHAR_BRACKETS_START:
    rest_of_expression =
        set_bracket_block(rest_of_expression, current_regex_block);
    break;
...
set_parentheses_block_return_status(char **p_parentheses_block_start,
  ...
  int size_of_block =
      get_bracket_block_end(*p_parentheses_block_start, FIRST_CHAR_IN_STR);
  ...
  set_parentheses_block_contents(&current_regex_block->regex_block_contents,
...
char *set_bracket_block(char *bracket_block_start,
  ...
  int bracket_end_index =
      get_bracket_block_end(bracket_block_start, FIRST_CHAR_IN_STR);

  current_regex_block->type = REGEX_TYPE_BRACKETS;
  current_regex_block->regex_block_contents.bracket_block.bracket_start =
      bracket_block_start[SECOND_CHAR_IN_STR];
  current_regex_block->regex_block_contents.bracket_block.bracket_end =
      bracket_block_start[bracket_end_index - 1];
...
int count_amount_of_regex_blocks(char *expression) {
  ...
  while (current_char_index < strlen(expression)) {
    ...
    case CHAR_BRACKETS_START:
      block_counter++;
      current_char_index =
          get_bracket_block_end(expression, current_char_index);
      break;
...
int get_bracket_block_end(char *expression, int index) {
  ...
  } while (current_char != '\0' && current_char_type != CHAR_PARENTHESES_END &&
