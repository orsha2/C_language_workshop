...
Flag load_flag_and_update_index(char **args, int *index,
                                Regex_And_Flags *my_regex_and_flags);
int get_flag_index(const char *current_flag);

Error_Code_t parse_cmd_args(int arg_num, char **args,
                            Regex_And_Flags **p_regex_and_flags,
                            char **p_expression, char **p_file_name) {
  int arg_index;
  Flag current_flag;
  ...
  while (arg_index < arg_num) {
    current_flag =
        load_flag_and_update_index(args, &arg_index, *p_regex_and_flags);
