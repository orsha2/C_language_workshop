...
bool check_if_matching_line(Line_Descriptor *line_desc,
                            Regex_And_Flags *my_regex_and_flags);
bool is_matching_expression(char *line, Regex_Block *regex_blocks,
                            int regex_block_num, bool *flags);
void print_if_matching(Line_Descriptor *line_desc, bool is_matching,
                       bool *flags);
bool compare_chars(const char first_char, const char second_char,
                   bool is_case_insensitive);
bool compare_strings(const char *first_str, const char *second_str,
                     bool is_case_insensitive);

Error_Code_t grep_execute_on_stream(FILE *input_stream,
                                    Regex_And_Flags *my_regex_and_flags) {
...
bool check_if_matching_line(Line_Descriptor *line_desc,
  ...
  for (size_t i = 0; i < strlen(line_desc->line); i++) {
    is_matching = is_matching_expression(
        line + i, my_regex_and_flags->regex_array,
...
bool is_matching_expression(char *line, Regex_Block *regex_blocks,
  ...
  case REGEX_TYPE_DOT:
    return is_matching_expression(line + 1, regex_blocks + 1,
                                  regex_block_num - 1, flags);

  case REGEX_TYPE_REGULAR_CHAR:
    if (compare_chars(*line, regex_blocks->regex_block_contents.regular_char,
                      flags[I_FLAG]) == false)
      return false;
    return is_matching_expression(line + 1, regex_blocks + 1,
                                  regex_block_num - 1, flags);
...
void print_if_matching(Line_Descriptor *line_desc, bool is_matching,
  ...
  if (flags[B_FLAG])
    printf("%ld: %s", line_desc->byte_counter - strlen(line_desc->line),
           line_desc->line);
...
bool compare_strings(const char *first_str, const char *second_str,
  ...
  if (is_case_insensitive)
    return (strcasecmp(first_str, second_str) ==
            0); // CHANGE THIS TO strcasecmp

  return (strcmp(first_str, second_str) == 0);
}

/*
Error_Code_t read_line_from_stream(FILE** input_stream, char* line_buffer, int
line_len)
{
        if (line_buffer == NULL)
